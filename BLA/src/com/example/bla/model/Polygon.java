	...
	public void draw(Canvas canvas) {
		...
		for (int i = 0; i < dots.size(); i++) {
			...
			if (i == (dots.size() - 1)) {
				...
				if( Math.abs(p2.x-p1.x) >radius*2 || Math.abs(p2.y-p1.y)>radius*2){// если расстояние между точкми больше чем диаметр точки
...
//			dots.get(i).draw(canvas);
	...
	private boolean lineInDrawRect(Dot dot1, Dot dot2){//если ТРУ-- рисуем, если ФОЛСЕ-- нет
